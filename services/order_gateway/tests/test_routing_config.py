...
def _reload_config(monkeypatch, enable_second: bool):
    ...
    if enable_second:
        ...
        monkeypatch.setenv("BINGX_SECOND_API_SECRET", "demo_secret_secondary")
    else:
        monkeypatch.delenv("BINGX_SECOND_API_KEY", raising=False)
        monkeypatch.delenv("BINGX_SECOND_API_SECRET", raising=False)
...
def test_default_routing_adds_secondary_when_env_present(monkeypatch):
    ...
    assert accounts[1].secret_key_env == "BINGX_SECOND_API_SECRET"
